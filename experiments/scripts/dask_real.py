...
def main(prefetch_storage, block_size, n_files, reps, types):
    ...
    for r in range(reps):
        ...
        for t in types:
            ...
            if t == "s3fs":
                seg = client.submit(segmentation_s3fs, (files, False, block_size, bfile=bfile))
            else:
                seg = client.submit(segmentation_prefetch, (header + files, False, block_size, prefetch_storage, bfile=bfile))
